        ...
        <div className="max-w-4xl mx-auto">
          <div className="space-y-4">
            {faqs.map((faq, idx) => (
              <div 
                key={idx}
                className={`bg-white dark:bg-[#0A101C] border transition-all duration-300 rounded-[32px] overflow-hidden ${openIndex === idx ? 'border-brand-primary/50 shadow-[0_20px_40px_rgba(40,168,224,0.1)]' : 'border-slate-100 dark:border-white/5 shadow-xl hover:border-slate-300 dark:hover:border-white/20'}`}
              >
                <button
                  onClick={() => setOpenIndex(openIndex === idx ? null : idx)}
                  className="w-full flex items-center justify-between p-8 text-left"
                >
                  <span className={`text-xl font-bold transition-colors ${openIndex === idx ? 'text-brand-primary' : 'text-slate-900 dark:text-white'}`}>
                    {faq.question}
                  </span>
                  <div className={`flex-shrink-0 ml-4 p-2 rounded-full transition-all ${openIndex === idx ? 'bg-brand-primary text-white rotate-180' : 'bg-slate-100 dark:bg-[#060D1A] text-slate-500 dark:text-white/60'}`}>
                ...
                <AnimatePresence>
                  {openIndex === idx && (
                    ...
                    >
                      <div className="px-8 pb-8 text-slate-600 dark:text-white/80 leading-relaxed text-lg border-t border-slate-100 dark:border-white/5 pt-6">
                        {faq.answer}
                      </div>
                    </motion.div>
                  )}
          ...
          <div className="mt-24 grid sm:grid-cols-3 gap-8">
            <div className="bg-white dark:bg-[#0A101C] border border-slate-100 dark:border-white/5 p-8 rounded-3xl text-center flex flex-col items-center">
                ...
                <p className="text-sm text-slate-500 dark:text-white/60 mb-4">Explore technical docs</p>
            ...
            <div className="bg-white dark:bg-[#0A101C] border border-slate-100 dark:border-white/5 p-8 rounded-3xl text-center flex flex-col items-center">
                ...
                <p className="text-sm text-slate-500 dark:text-white/60 mb-4">Talk to an expert</p>
            ...
            <div className="bg-white dark:bg-[#0A101C] border border-slate-100 dark:border-white/5 p-8 rounded-3xl text-center flex flex-col items-center">
                ...
                <h3 className="font-bold text-slate-900 dark:text-white mb-2">Call Support</h3>
                <p className="text-sm text-slate-500 dark:text-white/60 mb-4">+1 (800) RAPID-ERP</p>
