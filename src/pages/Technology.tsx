...
const ExpertiseCard = ({ title, desc, icon: Icon, color }: { title: string, desc: string, icon: any, color: string, key?: number | string }) => (
  <motion.div 
    ...
    viewport={{ once: true }}
    className="bg-slate-50 dark:bg-[#061121] border border-slate-100 dark:border-white/5 rounded-2xl p-6 md:p-8 flex items-start gap-6 hover:shadow-lg hover:shadow-brand-primary/10 hover:border-brand-primary/40 transition-all duration-300 group"
  >
    <div className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 bg-opacity-10 bg-slate-100 dark:bg-[#060D1A] group-hover:bg-opacity-20 transition-all ${color}`}>
    ...
    <div>
      ...
      <p className="text-sm font-medium leading-relaxed text-slate-600 dark:text-white/80">{desc}</p>
    </div>
  </motion.div>
);
...
const TechGroup = ({ title, technologies }: { title: string, technologies: { name: string, icon: ReactNode }[] }) => (
  <div className="mb-20">
    <div className="mb-8">
      <h3 className="inline-block text-2xl font-bold text-slate-900 dark:text-white border-b-2 border-brand-primary pb-2 mb-4">{title}</h3>
      <p className="text-slate-600 dark:text-white/80 text-sm font-medium">Reinvent the look and feel of web applications using JavaScript technologies along with HTML CSS improve aesthetics and enhance the digital experience.</p>
    </div>
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
      {technologies.map((tech, idx) => (
        <motion.div 
          ...
          viewport={{ once: true }}
          className="bg-slate-50 dark:bg-[#061121] border border-slate-100 dark:border-white/5 rounded-2xl p-6 flex flex-col items-center justify-center text-center gap-4 hover:border-brand-primary/40 hover:-translate-y-1 hover:shadow-xl hover:shadow-brand-primary/10 transition-all"
        >
          ...
          <span className="text-xs font-bold text-slate-700 dark:text-white/90 tracking-wide">{tech.name}</span>
        </motion.div>
      ))}
    </div>
  </div>
    ...
    >
      ...
      <section className="relative pt-32 pb-32 lg:pt-40 lg:pb-40 overflow-hidden flex items-center justify-center">
        ...
        <div className="w-full max-w-[1800px] mx-auto px-4 lg:px-8 xl:px-12 2xl:px-16 relative z-10 w-full text-left">
          ...
          >
            <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white mb-6">Technology</h1>
            <p className="text-lg text-slate-700 dark:text-white/90 leading-relaxed max-w-2xl">
      ...
      <section className="py-20 lg:py-28 relative">
        <div className="w-full max-w-[1800px] mx-auto px-4 lg:px-8 xl:px-12 2xl:px-16">
          <div className="flex justify-between items-end mb-10 border-b border-slate-200 dark:border-white/10 pb-4">
             <h2 className="text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Our Expertise</h2>
      ...
      <section className="py-12 pb-32 relative">
        <div className="w-full max-w-[1800px] mx-auto px-4 lg:px-8 xl:px-12 2xl:px-16">
          ...
          <TechGroup title="Frontend" technologies={[
            ...
            { name: "Express JS", icon: <span className="text-3xl font-bold text-slate-700 dark:text-white/90">ex</span> },
            { name: "Next JS", icon: <span className="text-3xl font-black text-slate-900 dark:text-white">N.</span> }
          ]} />
          ...
          <TechGroup title="Backend" technologies={[
            ...
            { name: "PHP", icon: <div className="bg-[#777BB4] text-slate-900 dark:text-white px-3 py-1 rounded-full text-xl font-black italic">php</div> },
            { name: "Next JS", icon: <span className="text-3xl font-black text-slate-900 dark:text-white">N.</span> }
          ]} />
          ...
          <TechGroup title="Mobile" technologies={[
            ...
            { name: "Apple", icon: <span className="text-3xl font-bold text-slate-700 dark:text-white/90"></span> },
          ...
          <TechGroup title="Others" technologies={[
            ...
            { name: "Kafka", icon: <span className="text-3xl font-bold text-slate-900 dark:text-white">Kafka</span> },
      ...
      <section className="py-20 lg:py-24 relative overflow-hidden bg-brand-primary">
        ...
        <div className="w-full max-w-[1800px] mx-auto px-4 lg:px-8 xl:px-12 2xl:px-16 relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
          <div>
              <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-4">Ready to build your custom application solution?</h2>
              <p className="text-slate-600 dark:text-white/80 font-medium text-lg">Send us your requirements on <span className="text-slate-900 dark:text-white font-bold">Info@gotorapid.com</span> or call <span className="text-slate-900 dark:text-white font-bold">+971 56 71 38 911</span></p>
           </div>
           <button className="bg-white dark:bg-[#0A101C] text-brand-primary px-8 py-4 rounded-full font-bold uppercase tracking-widest text-sm flex items-center gap-2 hover:shadow-xl hover:scale-105 transition-all">
